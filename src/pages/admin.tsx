    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <div class="flex flex-wrap items-start justify-between gap-4 mb-8">
        <div>
          ...
          <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900 mt-1">Regional Crop Health Overview</h1>
      ...
      <div class="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 card p-5 sm:p-6">
          <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fa-solid fa-map-location-dot text-orange-600"></i> Risk hotspots by block</h2>
          <div class="grid sm:grid-cols-2 gap-3">
            {HOTSPOTS.map((h) => {
              const rc = riskColor(h.riskLevel)
              return (
                <div class={`rounded-xl p-4 border ${rc.ring} ${rc.bg}`}>
        ...
        <div class="card p-5 sm:p-6">
          <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fa-solid fa-bell text-red-500"></i> Escalation queue</h2>
          <div class="space-y-3">
            {escalated.map((f) => {
              const rc = riskColor(f.riskLevel)
              return (
                <a href={`/diagnosis/${f.caseId}`} class="block rounded-xl border border-gray-100 p-3 hover:border-red-200 hover:bg-red-50/50 transition">
                  <div class="flex justify-between items-start">
      ...
      <div class="grid lg:grid-cols-2 gap-6 mb-8">
        <div class="card p-5 sm:p-6">
          <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fa-solid fa-chart-column text-agri-600"></i> Disease vs Pest split over time</h2>
          <canvas id="chart-split" height="220"></canvas>
        </div>
        <div class="card p-5 sm:p-6">
          <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fa-solid fa-chart-line text-tech-600"></i> Confidence trend (avg. monthly)</h2>
          <canvas id="chart-confidence" height="220"></canvas>
        </div>
        <div class="card p-5 sm:p-6 lg:col-span-2">
          <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fa-solid fa-cloud-bolt text-amber-600"></i> Outbreak risk vs. humidity correlation</h2>
          <canvas id="chart-weather" height="180"></canvas>
        </div>
      </div>

      {/* Continuous learning panel */}
      <div class="card p-5 sm:p-6 mb-8 bg-gradient-to-r from-eco-50 to-agri-50 border-eco-100">
        <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fa-solid fa-arrows-spin text-eco-600"></i> Continuous learning loop</h2>
      ...
      <div class="flex items-center justify-between mb-4">
        <h2 class="font-bold text-lg text-gray-900 flex items-center gap-2"><i class="fa-solid fa-address-book text-agri-600"></i> Farmer directory &amp; case status</h2>
      </div>
